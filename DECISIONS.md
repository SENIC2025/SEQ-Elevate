...
- **✅ EU data residency (resolved 2026-06, decider: SENIC)**: uploads must live in the **EU**. Chosen solution — a **Vercel Blob store in the `fra1` region (Frankfurt, Germany)**, which stores on **AWS eu-central-1**. Vercel supports per-store region selection specifically to meet data-residency requirements. This keeps the existing upload code unchanged (region is a store property, invisible to the SDK) and avoids a separate bucket.
  - **Action (Vercel dashboard, SENIC — one-time):** the region **cannot** be changed on an existing store, so **create a new Blob store with region = Frankfurt (`fra1`)** (dashboard region selector, or CLI `vercel blob store add --region fra1`), connect it to the project (Vercel sets `BLOB_READ_WRITE_TOKEN`), remove the old US store, and redeploy. **No migration needed** — the current store holds only staff-authored course media and nothing production-critical.
  - **Note**: Blob content is **staff-authored course material** (lesson videos/documents), not learner-uploaded personal data — learners upload nothing. Access is `public` with unguessable random-suffix URLs, which is appropriate for teaching material. If future content is learner-identifiable, switch the store to `private` access at creation (also immutable after creation) — a small, additive code change to serve blobs through an authenticated route.
  - Vercel Inc. is a US company, so keep Vercel's DPA/SCCs on file; the data itself resides in Germany.

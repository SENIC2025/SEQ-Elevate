...
### Comp Card template — editable wording (CMS) (2026-06-16)
Final third of the course-lifecycle criterion. `getCompCardTemplate` already existed but the Comp Card form was hardcoded to the message catalogue — the CMS function's output was never actually used, so "editing the template" changed nothing.
- `[BUILD]` Migration `comp_card_template` — `Project.compCardTemplate Json?` holds a per-locale override `{ en: { fields: { key: { label, placeholder } }, hidden: [], order: [] } }`. Null = bundled copy.
- `[BUILD]` `src/lib/cms/comp-card-template.ts` (pure, unit-tested) — `applyTemplateOverride` rewords / reorders / hides; **unknown keys are dropped** (the override is untrusted JSON, so it can't inject a field the form can't store); a blank label falls back rather than rendering a nameless field; hiding *every* field falls back to the base rather than yielding an empty form. `pickOverride` does locale → en → none.
- `[BUILD]` `src/lib/cms/comp-card-overlay.ts` (`server-only`) applies it; `getCompCardTemplate` now overlays; new `getBaseCompCardTemplate` returns the un-overridden template.
- `[BUILD]` The Comp Card **form and the printable/PDF version** both consume the template, so wording, order and hidden fields stay consistent between what a learner fills in and what they take away.
- `[BUILD]` `src/app/actions/comp-card-template.ts` — `getCompCardEditorState` (built from the **bundled** template so a hidden field can still be un-hidden — editing from the effective template would make hiding a one-way door), `saveCompCardTemplate` (per-language merge; refuses hiding everything), `clearCompCardTemplate`. Audit-logged.
- `[BUILD]` `CompCardTemplateEditor` in the content editor: per-language, reword question + hint, reorder (↑/↓), show/hide, save, revert. States plainly that **field keys are fixed** (each maps to a stored column) so adding a genuinely new field is a schema change — no button that silently loses data.
- `[FIX]` Two bugs caught before shipping: (1) editing from the effective template would have made a hidden field unrecoverable — fixed by loading the editor from the bundled template; (2) `undefined` doesn't clear a Prisma JSON column (`undefined` = "leave unchanged"), so reverting the last language now writes `Prisma.DbNull`.
- `[VERIFY]` 8 new unit tests + 3 DB-backed E2E: an editor rewords a field → persisted to `Project`, audited, and the **learner sees the new wording** on their Comp Card; reverting restores the bundled copy; a guest gets no editor controls. Comp Card page still passes axe (WCAG 2.2 AA). Build ✓ lint ✓ types ✓ **45 unit ✓ 41 E2E ✓**.
- `[NOTE]` **Course-lifecycle acceptance criterion is now complete**: create a course, publish/unpublish it, and edit the Comp Card template — all without a developer. The remaining large item is the interactive **structure editor** (authoring simulations, branching scenarios and assessments in the DB), which is a roadmap increment, not an acceptance gap.

### Interactive structure editor — A: read/play path (2026-06-16)
The roadmap "structure editor": author simulations, branching scenarios and assessments for CMS-created courses in the DB, so a course can be built end-to-end without code. Shipped in vertical slices; this is the read/play foundation.
- `[BUILD]` Migration `lesson_structure` — `Lesson.structure Json?` holds a stage's interactive structure. **Design**: the structural facts (which option is `isBest`, a branch's `quality`, an assessment's `correctOptionId`) are **language-neutral and stored once**; only human text is per-locale `{ en, de, el }` with fallback. Deliberate — if correctness could differ between languages, a translation slip would mis-grade a whole cohort. Mirrors the bundled model (structure in code, text in messages), not the narrative overlay (independent per-locale copies).
- `[BUILD]` `src/lib/cms/structure.ts` (pure, unit-tested) — `StoredSimulation | StoredScenario | StoredAssessment` types, `resolveStructure(structure, locale)` → the matching `CourseStage` slice, and `validateStructure` (≥2 options, exactly one best, correct-id must be a real option, text in ≥1 language). 15 unit tests, incl. "the correct option is identical across locales".
- `[BUILD]` `db-course.ts` now assembles the full 7-stage sequence in canonical order: narrative stages always render; the three interactive stages render **only once their structure is authored** (else omitted). The three stage components already render purely from stage data, so a DB-authored stage plays and **grades** identically to a bundled one — no player changes.
- `[FIX]` Latent bug uncovered by the first end-to-end CMS walk: a CMS course built its **reflection** stage as narrative `blocks`, but the player's `ReflectionStage` needs a `reflection` object and returns `null` without it — so the walk silently stalled with no Continue. Never caught because no prior test walked a CMS course to completion. `db-course` now builds a proper reflection stage (authored copy as the intro + generic private journaling prompts).
- `[VERIFY]` 15 new unit tests + a DB-backed E2E that seeds a published CMS course with authored narrative **and all three interactive stages**, then walks all seven: the authored simulation prompt renders and the best option gives its feedback; the scenario root+follow-up play; the assessment grades; the course **completes** and the enrollment records >5 stages. Build ✓ lint ✓ types ✓ **57 unit ✓ 42 E2E ✓**.
- `[NOTE]` Next slices: authoring UIs — simulation (B), scenario tree (C), assessment (D) — writing `Lesson.structure` with `validateStructure` gating publish.

### Interactive structure editor — B: authoring UI (2026-06-16)
The authoring surface for the read/play path shipped in A: content editors now build simulations, branching scenarios and assessments for a CMS course, no code.
- `[BUILD]` `src/app/actions/structure.ts` — ADMIN/CONTENT_EDITOR only: `listAuthorableCourses` (CMS courses only — bundled courses own their structure in code, and `getCourse` would ignore a CMS override, so they're excluded up front), `getStructure`, `saveStructure` (validated with `validateStructure`; refuses a kind/stage mismatch and non-CMS courses; returns the issue list so the UI can warn), `clearStructure` (removes the stage). Audited.
- `[BUILD]` `StructureAuthor` component — course + stage + language selectors, then a kind-specific editor: **simulation** (options with a star for the single best + per-option feedback), **scenario** (first choices each with a quality tag and nested follow-ups), **assessment** (questions with a radio to mark the correct answer). Per-locale text via a shared `LocField`; validation issues surfaced inline before save. Wired into the Content Editor.
- `[FIX]` A real cross-course bug caught by a parallel-run flake, not just papered over in the test: changing the course selector reloads the draft asynchronously, but Save read the new `slug` immediately — so saving during that window would write the *previous* course's draft onto the new course. Fixed by folding the draft and its owning `(course, stage)` key into one state unit and deriving `ready` from the key; Save is disabled until the load for the current selectors resolves. (Also removed a stray `set-state-in-effect`.)
- `[FIX]` Removed a latent footgun in the E2E suite: the first describe's `afterAll` called `prisma.$disconnect()` on the shared client, which breaks any describe that runs later on the same Playwright worker. Prisma releases the pool on worker exit, so it was redundant.
- `[VERIFY]` 2 authoring E2E (editor fills the simulation editor → validation gates the empty save, then it persists with exactly one best option + audit; editor authors an assessment → the ticked answer is stored as `correctOptionId`). Local full-suite verification was blocked by a wedged local Docker daemon (unrelated to the code); CI's isolated Postgres is the authoritative run. Build ✓ lint ✓ types ✓ 57 unit ✓.

### Demo CMS course seeded on staging (2026-06-16)
Client need: a ready-made, fully CMS-created course so clients can open the platform and try one end-to-end.
- `[BUILD]` `prisma/seed-demo.mjs` — **idempotent** upsert of one polished CMS course, **"Demo · Speaking up when something feels off"** (`demo-speaking-up`): three authored narrative stages (context/concept/behaviour, with callout + compare + list blocks), an authored **simulation** (I-statement vs blame vs stay-silent), a **branching scenario** (private vs public, each with follow-ups and quality tags), and a two-question **assessment** — plus the auto reflection stage. Published.
- `[BUILD]` Wired into `vercel-build` (`migrate-deploy → seed-demo → next build`). It runs in **Vercel's build environment**, so it uses the deploy's own Neon credential — no secret is handled outside Vercel. **Idempotent** (upsert by slug + stage), so redeploys refresh in place. **Never fails the build** (errors are logged and swallowed, always exits 0).
- `[SECURITY]` Guarded by `DEMO_LOGIN_DISABLED=true` — the same kill-switch that disables demo sign-in on a real production domain (D16). So the demo course rides the demo deploy and disappears on real prod, no separate flag to remember.
- `[VERIFY]` Ran the seed against a local DB, validated all three structures (exactly one best option; every assessment `correctOptionId` maps to a real option; ≥2 scenario choices), then **walked the whole course as a signed-in learner** through a throwaway Playwright spec: narrative → simulation (correct feedback) → scenario (root + follow-up) → reflection → assessment (graded) → **Course complete**. Lint ✓ build ✓. The push auto-deploys and seeds Neon.
- `[NOTE]` Authored EN; DE/EL fall back to English at render time (the resolver's per-field fallback) until a translator fills them in — which itself demos the graceful-fallback behaviour.

### Demo course translated to German + Greek (2026-06-16)
The build-seeded demo course is now trilingual — every learner-facing string authored in EN + DE + EL.
- `[BUILD]` `prisma/seed-demo.mjs` — all content (course meta, three narrative stages incl. callout/compare/list blocks, the simulation, the branching scenario with follow-ups, the assessment) now carries `{ en, de, el }`. **Structural facts (option ids, isBest, quality, correctOptionId) are unchanged and shared across languages** (D20) — only text was added, so a translation can't alter grading.
- `[FIX]` `db-course.ts` localised the generic **system** slots a CMS author never types: the completion heading ("Course complete" → "Kurs abgeschlossen" / "Το μάθημα ολοκληρώθηκε") and the reflection stage's title, intro and journaling prompts — per locale, with EN fallback. Previously these were hardcoded English, so a translated CMS course would have leaked English on the reflection + completion screens. This benefits **every** CMS course, not just the demo.
- `[VERIFY]` Re-seeded locally, then walked the course end-to-end **in German** (narrative → simulation feedback → scenario root+follow-up → localised reflection prompts → graded assessment → "Kurs abgeschlossen") and confirmed **Greek** strings render through the simulation. Build ✓ lint ✓ types ✓. The push auto-deploys and re-seeds Neon in all three languages.
- `[NOTE]` Translations are natural (informal "du" / modern Greek), matching the warm coaching register — not literal. Idempotent upsert, so the re-seed refreshes the existing course in place.

### Go-live prep — legal pages surface + global footer (2026-06-16)
Delivers acceptance item O7 (the legal-page rendering surface) toward production launch.
- `[BUILD]` `/[locale]/legal/[doc]` renders four documents: **Cookies** and **Accessibility statement** written in full (factual, SENIC-ownable), **Privacy policy** and **Terms of use** as structured scaffolds pre-filled with the platform's technical facts and every consortium-supplied field boxed as **"To be completed before launch — …"** so nothing ships looking finalised. Content lives in `src/data/legal.ts`, per-locale with EN fallback (a "translation being finalised" note shows on DE/EL until their official text lands).
...
### Go-live prep — security review + fixes (2026-06-16)
A focused security pass (authz/RBAC, IDOR, injection, untrusted input) over this session's server actions and the go-live work. Two findings, both fixed; the rest of the surface reviewed clean (admin/course/structure/comp-card/lesson actions all role-gate before writing; Prisma is parameterised; the legal page renders auto-escaped JSX; the magic-link email interpolates only the server-generated URL; uploads are staff-gated + type/size-capped; CMS read helpers take `projectId` from a constant, so no cross-tenant IDOR).
- `[SECURITY]` **HIGH — privilege escalation via an exported internal helper.** `grantRoles(userId, roles)` was `export`ed from a `"use server"` module, which registers it as a public server-action endpoint — callable by anyone, with **no auth check and bypassing every demo gate**. A signed-in learner could have called `grantRoles(theirId, ["ADMIN"])` to self-escalate to project admin, **even on real production**. Fixed: made it module-private (it's only ever called by `demoSignIn`). Lesson: any `export` in a `"use server"` file is an anonymous HTTP endpoint.
- `[SECURITY]` **MEDIUM — source-visible default that granted ADMIN.** Demo sign-in fell back to the hardcoded code `"elevate-demo"` on *any* host if `DEMO_ACCESS_CODE` was unset, so a real-prod deploy that forgot the `DEMO_LOGIN_DISABLED` kill-switch would accept demo admin login with a public code. Fixed to **fail safe**: the public code is honoured **only on the demo deployment** (matched via the Vercel-set `VERCEL_PROJECT_PRODUCTION_URL`/`VERCEL_URL`, which a client can't control); on any other host demo login is **off unless `DEMO_ACCESS_CODE` is explicitly set**. The live demo is unaffected (its deploy URL matches); real prod is now safe by default. E2E + local set the code explicitly.
- `[VERIFY]` Build ✓ lint ✓ types ✓ 57 unit ✓ 46 E2E ✓ (demo-access sign-in still passes via the explicit code).

### Correction — demo-login finding-2 fix reverted (auto-detection unreliable) (2026-06-16)
The MEDIUM finding-2 fix (honour the public demo code only on the demo host, via `VERCEL_PROJECT_PRODUCTION_URL`) **broke demo login on staging** — those Vercel system env vars are not reliably populated in the Next.js server-action runtime, so `effectiveAccessCode()` returned null and the live client demo showed "Demo access is turned off." Reverted to the simple `DEMO_ACCESS_CODE ?? "elevate-demo"` default.
- `[NOTE]` There is no reliable, non-spoofable runtime signal to auto-distinguish the demo deploy from real production — the only trustworthy signal is an explicit env var, and **`DEMO_LOGIN_DISABLED=true` already is that var** (it turns the whole demo-login path off, checked first in `demoSignIn`, and is documented as the mandatory #1 launch setting). So finding 2 is mitigated by the kill-switch rather than by fragile auto-detection. GO-LIVE.md updated to say so plainly.
- `[SECURITY]` **Finding 1 (the real HIGH — `grantRoles` was a public unauthenticated privilege-escalation endpoint) stays fixed**: it remains module-private. That was the actual exploit; it's closed.
- `[VERIFY]` Build ✓ lint ✓ types ✓, demo-access E2E ✓, and demo login confirmed working again on staging.

### Go-live prep — Sentry error monitoring wired (inert until DSN) (2026-06-16)
Error monitoring is now wired and ready; activation is just setting the DSN env vars + redeploy.
- `[BUILD]` `@sentry/nextjs` (v10, officially supports Next 16). `src/instrumentation.ts` (server + edge `register()` + `onRequestError`) and `src/instrumentation-client.ts` (browser) init Sentry **only when a DSN is present**; `captureException` added to both error boundaries (`global-error.tsx`, `[locale]/error.tsx`).
- `[BUILD]` **Fully gated + zero-cost when off**: every Sentry call is behind an env-var check with a **dynamic** `import("@sentry/nextjs")`. `NEXT_PUBLIC_SENTRY_DSN` is inlined at build, so with no DSN the client branch is statically dead and the Sentry SDK is **tree-shaken out of the learner bundle** entirely; the server never imports or inits it.
- `[BUILD]` **Privacy-conservative defaults** for a vulnerable-youth platform: `sendDefaultPii: false`, `tracesSampleRate: 0` (no performance tracing), and **no session replay** (replaysSessionSampleRate/OnError = 0). Documented that replay must not be enabled without a DPIA.
- `[NOTE]` Deliberately did **not** wire `withSentryConfig` / source-map upload — that couples to the Turbopack build and needs `@sentry/cli`'s postinstall binary. So `@sentry/cli` is marked `allowBuilds: false` in `pnpm-workspace.yaml` (its build is unnecessary without upload). This was also the fix for a broken `pnpm install` (the unresolved `allowBuilds` placeholder pnpm auto-inserted made install exit 1, which broke the E2E webserver).
- `[VERIFY]` Build ✓ (exit 0, **no Sentry build warnings** — confirms the runtime-only wiring leaves the build untouched) lint ✓ types ✓ 57 unit ✓ 46 E2E ✓, all with no DSN set. Activation steps in GO-LIVE.md §6.

### Go-live prep — production hosting/domain/environment decisions recorded (2026-08-21)
Client (SENIC principal) settled the three operational go-live decisions after a review of what each entails. No functional code change beyond the metadata base; the value is accurate decisions + an updated runbook.
- `[DECISION]` **Hosting = Vercel** (D22), *not* the Hetzner VPS named in the signed proposal §4 / D1 / D5. Rationale: the app is architected for Vercel (Blob uploads, serverless Server Actions, auto-migrate on deploy, EU/FRA1, one-click rollback); a VPS would be a re-architecture + self-managed ops for no pilot benefit. Flagged the **commercial implication** — Vercel is a recurring SaaS cost profile vs D1's fixed-VPS budget (still within the €5,500 lump at pilot scale) — as a conscious departure to confirm with the consortium. Hostinger kept for the marketing site.
- `[DECISION]` **Environment = reuse the demo Vercel project as production** (D23), not a separate clean project. SENIC recommended isolating real (incl. under-18) learner data; principal chose reuse for speed. Recorded the trade-off and the mandatory **cutover hygiene** (GO-LIVE.md §2a): `DEMO_LOGIN_DISABLED=true` + rotate `AUTH_SECRET` + purge demo data before the first real learner.
- `[DECISION]` **Production host = apex `seq-elevate.eu`** (O1 closed) with `www` → apex redirect. Documented the apex-specific DNS (A record, not CNAME) and that email records are unaffected.
- `[FIX]` `layout.tsx` `metadataBase` fallback updated `seq-elevate-demo.vercel.app` → `https://seq-elevate.eu` (still prefers `AUTH_URL`, which every real env sets). Only affects absolute OG/social URLs.
- `[NOTE]` **Codebase is domain-clean** — audited for host assumptions (`vercel.app`, `VERCEL_URL`, hardcoded `https://`, `trustHost`, Plausible domain): the magic-link base and everything host-dependent derive from `AUTH_URL`/request host, so the domain switch needs **no code change** beyond the cosmetic metadata fallback. `metadataBase` was the only host literal.
- `[NOTE]` **Hyphen flag**: brand site referenced as `seqelevate.eu` (no hyphen) vs platform `seq-elevate.eu` (hyphen) — raised in DECISIONS O1 + GO-LIVE §3 for the consortium to reconcile.

### Go-live prep — demo-data purge tool + capacity/cost review (2026-08-21)
Client confirmed: demo no longer needed, go straight to production; future client presentations run on the live platform (real logins). This removes the only argument against reusing the demo project (D23) — no competing "demo mode" need.
- `[BUILD]` `prisma/purge-demo.mjs` (npm `db:purge-demo`) — the D23 §3 cutover step made real. **Dry-run by default**, `--confirm` to execute. Removes the demo course `demo-speaking-up` (its `Lesson` rows are string-keyed by `courseSlug`, so a `Course` delete does *not* cascade to them — deleted explicitly) and the four one-click demo accounts; a `User` delete cascades its data and SetNulls the audit actor (anonymised, not lost). Never touches project/orgs/cohorts/bundled courses. `--all-users` (opt-in, `KEEP_EMAILS` survives) for a full clean slate — safe at cutover since no real learners exist yet. Wrapped in a `$transaction`.
- `[VERIFY]` `node --check` ✓; field names (`Lesson.courseSlug/stageKey`, `Course.slug`) verified against `schema.prisma`. Not run against a live DB by design — the dry run is the first thing the operator runs.
- `[NOTE]` **Capacity for the real pilot (120 learners, 6–10 courses):** well within a small managed stack. 120 users' records are tens of MB incl. audit; serverless compute + pooled Neon handle the concurrency trivially. The only variable cost is **video egress** from Blob — and the player already supports **YouTube/Vimeo embeds** (`InteractiveVideoPlayer`, `VideoBlockAuthor`), so teaching video can live off-platform at zero storage/bandwidth cost; Blob then only holds small PDFs. Recommend **Vercel Pro** (commercial use requires it) + **Neon Launch** (autoscale + PITR) for production.

### Go-live prep — cost lifecycle decided (Neon Launch → Free when passive) (2026-08-21)
Client raised the project's cost shape: short active pilot (~120 learners, ~1 year) then a long passive phase; wants lowest total cost without risking reliability during the pilot. Reconciled against Neon's live pricing page (screenshot: Free $0 / Launch usage-based ~$15/mo / Scale ~$701/mo).
- `[DECISION]` **D24 — active on Neon Launch, downgrade to Neon Free when passive.** Vercel Pro is sunk (kept regardless), so the only new recurring cost is the DB. Launch (~$15/mo usage-based, the pilot's "intermittent load + 1 GB" profile) during the pilot for PITR + no CU-hr cap + no cold-start lag; downgrade to Free ($0) when passive — a **billing toggle, not a migration** (data is tens of MB, inside Free's 0.5 GB). Total new lifecycle cost ≈ €150–200 then ≈ €0.
...
### Go-live prep — admin onboarding: ADMIN_EMAILS allow-list bootstrap (2026-08-21)
Answered "how do clients become admins / sign up?" — reframed and wired the missing piece. Passwordless means sign-in already IS sign-up; the real gap was a production admin bootstrap (previously only the hardcoded demo emails auto-elevated).
- `[BUILD]` `auth.ts`: new `ADMIN_EMAILS` env allow-list (comma/space/newline separated, case-insensitive). `events.signIn` now grants **ADMIN + CONTENT_EDITOR** to any email on the list, decoupled from demo profiles. Roles remain server-granted — no self-selection anywhere.
- `[SECURITY]` **Demo-profile role grants gated to showcase** (`DEMO_LOGIN_DISABLED !== "true"`). Previously the demo emails (e.g. `stefan@senic.org`) auto-received their roles on *any* host; now on real prod only `ADMIN_EMAILS` governs admin — closes an implicit hardcoded-email admin path in production.
- `[NOTE]` The "admin adds their org's people" flow was already built (`admin.ts addMember(email, role)` + cohort/org assignment) — trainers/teachers/learners/other-admins by email; they sign in via magic link and land in their role. Only the first-admin bootstrap was missing.
- `[VERIFY]` `tsc --noEmit` ✓, eslint ✓. Documented: D25 (decision), GO-LIVE §1 env row + §8 onboarding model, `ADMIN_EMAILS` set in Vercel before go-live.

### Maintenance package — 30-day off-site backups + hosting-substitution note (2026-09-07)
Client raised the Item 2 (€5,500, 5-year hosting/maintenance) invoice and asked to confirm the 10-point package. Reviewed honestly: 3 items met, several activation-pending, and three diverged from the contract wording (Hetzner/Hostinger→Vercel, S3-compatible→Blob, 30-day backups). Built the two things that move divergent items to fulfilled/agreed.
- `[BUILD]` `.github/workflows/db-backup.yml` — **daily `pg_dump` → S3-compatible EU bucket, 30-day prune** (+ manual dispatch). No-op with a warning until secrets are set; never silently fails. Fulfils Item 2 §7 ("daily backups, retained 30 days"), which Neon Launch's ~7-day PITR alone did not meet. YAML validated. Restore + setup in `BACKUP.md`.
- `[BUILD]` `HOSTING-SUBSTITUTION.md` — proposed amendment for the consortium: documents the Vercel+Neon(Frankfurt)+Blob(Frankfurt) delivery vs the proposal's Hetzner/Hostinger + S3-compatible wording, with a full 10-item status table and a sign-off block. EU/Germany residency is preserved; only the named providers differ (D22). Asks the consortium to approve the substitution so the Item 2 invoice matches reality.
- `[NOTE]` **Go-live date for the invoice is not stampable from here** — the authoritative date is the Vercel production deployment that first served `seq-elevate.eu` with `DEMO_LOGIN_DISABLED=true`. Engineering go-live work landed 2026-08-21..23; production-mode serving verified this session; today 2026-09-07. Use the Vercel deployment/domain timestamp on the invoice.
- `[NOTE]` Did **not** rubber-stamp "package fulfilled": items 8–10 are prospective 5-year commitments (begin at go-live), item 5 needs a Sentry DSN + an uptime monitor, item 6 needs the Plausible domain env.
